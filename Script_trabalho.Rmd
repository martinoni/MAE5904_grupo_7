...
A implementação do método foi feita através do Python. Os arquivos relacionados estão disponíveis no Github (cujo link está no fim desse relatório). Para mais detalhes, veja os seguintes arquivos:
...
Como esses resultados foram semelhantes aos do modelo de regressão logística, que possui uma melhor interpretabilidade e menor complexidade de implementação (e tempo de ajuste), acabamos por não escolher as redes neurais artificiais como modelo final do projeto.
