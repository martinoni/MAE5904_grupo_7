...
Note que tirando profissionalSaude, todas as variáveis se mostraram significativas a um nível de $5\%$, então serão mantidas (manteremos profissional de saúde pois sua retirada não muda as métricas, e pode ser uma informação útil). Aqui vamos também checar por multicolinearidade.
...
```{r}
pred.rl.treino = predict(fit.rl, newdata = dados.treino %>% dplyr::select(-c('resultadoTeste', 'idade')), type = 'response')
```
