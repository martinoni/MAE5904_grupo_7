...
dados.treino$resultadoTeste = relevel(treino$resultadoTeste, ref = 'Negativo')
...
Notemos que há evidências de multicolinearidade usando os critérios usuais (VIF maiores que 5), então retiraremos _faixaetaria_ do modelo. \newline

```{r}
fit.rl = glm(resultadoTeste ~ ., family = binomial,data = subset(dados.treino, select = -faixaetaria))
summary(fit.rl)
```

Notemos ainda que agora _profissionalSaude_ deixou de ser significativa a $5\%$, então também retiraremos do modelo final.

```{r}
fit.rl = glm(resultadoTeste ~ ., family = binomial,data = subset(dados.treino, select = -c(faixaetaria, profissionalSaude)))
summary(fit.rl)
```
...
```{r}
pred.rl.treino = predict(fit.rl, newdata = dados.treino %>% dplyr::select(-c('resultadoTeste')), type = 'response')
```
...
pr.train.perf = ROCR::performance(pr.train, measure = 'fpr', x.measure = 'tpr')
plot(pr.train.perf)
...
O modelo conseguiu classificar corretamente $72\%$ (sensibilidade) das pessoas que tiveram resultado positivo no exame, e classificou corretamente $11\%$ das pessoas que tiveram resultado negativo. \newline
...
A sensibilidade novamente é de $73\%$ enquanto a especificidade é de $10\%$, uma performance parecida com a dos dados de treino.
...
Notemos, então, que a sensibilidade nos dados de teste foi de $76\%$ e a especificidade de $11\%$. 
...
```{r, results='show'}
Acurácia = c(31.92, 31.54, 27.78)
Sensibilidade = c(72.61, 73.01, 76.68)
Especificidade = c(10.28, 09.95, 10.82)
...
```{r}
tic('RL fitting')
fit.rl2 = glm(resultadoTeste ~ ., family = binomial,data = dados.treino)
toc()
```
...
```{r, results='show'}
Acurácia = c(49.98, 49.71, 42.45)
Sensibilidade = c(81.39, 81.44, 79.31)
Especificidade = c(33.27, 33.19, 29.67)
med_rl = cbind(Acurácia, Sensibilidade, Especificidade)
rownames(med_rl) = c('Treino', 'Validação', 'Teste')
med_rl %>% 
